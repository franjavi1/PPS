...
export default function ComisionesAsignaturas() {
  ...
  async function cargarDatos() {
    try {
      ...
      const [resRegs, resPA, resAsig, resPlanes, resAulas, resComs, resSedes] = await Promise.all([
        ...
        aulaService.obtenerTodas(), comisionService.obtenerTodas(), sedeService.obtenerTodas(),
      ]);
      setRegistros(resRegs.data || []); setPlanesAsignaturas(resPA.data || []); setAsignaturas(resAsig.data || []);
      setPlanes(resPlanes.data || []); setAulas(resAulas.data || []); setComisiones(resComs.data || []); setSedes(resSedes.data || []);
    } catch (err) {
  ...
  async function guardarRegistro(e) {
    ...
    if (!paId || !aulaId || !comisionId) return setErrorFormulario("Debe completar todos los select obligatorios");

    const payload = {
      plan_asignaturas_id: paId, aula_id: aulaId, comision_id: comisionId, nombre: formulario.nombre.trim(),
      modalidad: formulario.modalidad.trim(), cupo_maximo: Number(formulario.cupo_maximo), estado: formulario.estado, usuario_accion: 1,
    };
    try {
      setErrorFormulario("");
      if (editandoId) await comisionAsignaturaService.actualizar(editandoId, payload);
      else await comisionAsignaturaService.crear(payload);
  ...
  async function eliminarRegistro(id) {
    if (!confirm("¿Seguro que querés quitar este registro?")) return;
    try {
      await comisionAsignaturaService.eliminar(id); await cargarDatos();
    } catch (err) {
  ...
  const registrosFiltrados = registros.filter((r) => {
    ...
    return String(r.nombre || "").toLowerCase().includes(term) || (mapas.planesAsignaturas[r.plan_asignaturas_id] || "").toLowerCase().includes(term);
  });
  ...
  return (
    <div className="min-h-screen bg-slate-100">
        ...
        <section className="bg-white rounded-2xl shadow-md border border-slate-200 p-8">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6 mb-8">
            <div className="flex items-start gap-5">
              ...
              <div>
                <h1 className="text-4xl font-extrabold text-slate-800">Asignaturas de Comisión</h1>
                <p className="text-slate-500 mt-2">Consulta y gestiona las asignaturas impartidas en cada comisión.</p>
              </div>
            </div>
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              {hasPermission(currentUserRole, "crear") && (
                <button onClick={() => { setFormulario(formularioInicial); setEditandoId(null); setMostrarModal(true); }} className="flex items-center justify-center gap-2 bg-red-700 text-white px-6 py-3 rounded-lg font-bold hover:bg-red-800"><PlusCircle size={22} /> Nueva Asignatura</button>
              )}
            </div>
          </div>
          <div className="relative w-full md:w-96 mb-8">
            <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={22} />
            <input type="text" value={busqueda} onChange={(e) => setBusqueda(e.target.value)} placeholder="Buscar por asignatura" className="w-full h-14 pl-12 pr-4 border border-slate-300 rounded-lg text-slate-700 focus:outline-none" />
          </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead className="bg-slate-50">
                <tr className="border-b border-slate-200">
                  ...
                  <th className="px-5 py-4 text-slate-700 font-bold">Nombre / Modalidad</th>
                  <th className="px-5 py-4 text-slate-700 font-bold">Plan Materia Sede</th>
                ...
                {cargando ? (
                  ...
                  registrosFiltrados.map((r) => (
                    <tr key={r.id_comision_asignatura} className="border-b border-slate-200 hover:bg-slate-50">
                      <td className="px-5 py-5 font-semibold text-slate-800">{mapas.comisiones[r.comision_id]}</td>
                      <td className="px-5 py-5 text-slate-700 font-bold text-red-600">{r.nombre} ({r.modalidad})</td>
                      <td className="px-5 py-5 text-slate-600">{mapas.planesAsignaturas[r.plan_asignaturas_id]}</td>
                      <td className="px-5 py-5 text-slate-600">{mapas.aulas[r.aula_id]}</td>
                      <td className="px-5 py-5 text-center">
                        <div className="flex justify-center gap-3">
                          {/* Si no tiene permisos de rol, ocultamos/deshabilitamos el elemento para que no intente la llamada. */}
                          <button onClick={() => { setFormulario({ plan_asignaturas_id: r.plan_asignaturas_id, aula_id: r.aula_id, comision_id: r.comision_id, nombre: r.nombre || "", modalidad: r.modalidad || "", cupo_maximo: r.cupo_maximo || "", estado: r.estado }); setEditandoId(r.id_comision_asignatura); setMostrarModal(true); }} disabled={!hasPermission(currentUserRole, "editar")} className={`text-blue-600 hover:text-blue-800 font-semibold flex items-center gap-1 ${hasPermission(currentUserRole, "editar") ? "" : "opacity-50 cursor-not-allowed"}`}><Pencil size={16} /> Editar</button>
                          {/* Si no tiene permisos de rol, ocultamos/deshabilitamos el elemento para que no intente la llamada. */}
                          <button onClick={() => eliminarRegistro(r.id_comision_asignatura)} disabled={!hasPermission(currentUserRole, "eliminar")} className={`text-red-600 hover:text-red-800 font-semibold flex items-center gap-1 ${hasPermission(currentUserRole, "eliminar") ? "" : "opacity-50 cursor-not-allowed"}`}><Trash2 size={16} /> Eliminar</button>
                        </div>
                      </td>
                    </tr>
                  ))
                ) : (
                  <tr><td colSpan="5" className="text-center py-10 text-slate-500">No se encontraron comisiones asignaturas.</td></tr>
                )}
              </tbody>
            </table>
          </div>
      ...
      <ComisionAsignaturaModal mostrarModal={mostrarModal} cerrarModal={() => setMostrarModal(false)} editandoId={editandoId} formulario={formulario} manejarCambio={manejarCambio} errorFormulario={errorFormulario} guardarRegistro={guardarRegistro} planesAsignaturas={planesAsignaturas} aulas={aulas} comisiones={comisiones} />
    </div>
  );
}
