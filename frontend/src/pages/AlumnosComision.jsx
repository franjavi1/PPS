...
import { alumnoComisionService } from "../services/alumnoComisionService";
import { comisionAsignaturaService } from "../services/comisionAsignaturaService";
import { condicionAcademicaService } from "../services/condicionAcademicaService";

const formularioInicial = {
  legajo_id: "",
  comision_id_comision: "",
  condicion_academica_id: "",
  nota_final: "",
  presentismo_porc: "",
};

function AlumnosComision() {
  const [registros, setRegistros] = useState([]);
  const [legajos, setLegajos] = useState([]);
  const [comisionesAsignaturas, setComisionesAsignaturas] = useState([]);
  const [condicionesAcademicas, setCondicionesAcademicas] = useState([]);
  ...
  async function cargarDatos() {
    try {
      ...
      const [
        ...
        alumnoComisionService.obtenerTodos(),
        apiRequest("/legajos"),
        comisionAsignaturaService.obtenerTodos(),
        condicionAcademicaService.obtenerTodas(),
      ]);

      setRegistros(respuestaAlumnos.data || []);
      setLegajos(respuestaLegajos.data || []);
      setComisionesAsignaturas(respuestaComisiones.data || []);
      setCondicionesAcademicas(respuestaCondiciones.data || []);
    } catch (err) {
      setError(err.message || "No se pudieron obtener los alumnos por comision");
    } finally {
      setCargando(false);
    }
  }
  ...
  const mapas = useMemo(() => {
    return {
      ...
      condiciones: condicionesAcademicas.reduce((acc, condicion) => {
        acc[condicion.id_condicion_academica] = condicion.condicion;
        return acc;
      }, {}),
    };
  }, [legajos, comisionesAsignaturas, condicionesAcademicas]);
  ...
  function editarRegistro(registro) {
    setFormulario({
      ...
      comision_id_comision: String(registro.comision_id_comision || ""),
      condicion_academica_id: String(registro.condicion_academica_id || ""),
      nota_final: registro.nota_final ?? "",
      presentismo_porc: registro.presentismo_porc ?? "",
    });
    setEditandoId(registro.id_historia_alumnos);
    setErrorFormulario("");
    setMostrarModal(true);
  }
  ...
  async function guardarRegistro(e) {
    ...
    const payload = {
      ...
      comision_id_comision: Number(formulario.comision_id_comision),
      condicion_academica_id: Number(formulario.condicion_academica_id),
      nota_final: formulario.nota_final === "" ? null : Number(formulario.nota_final),
      presentismo_porc:
        formulario.presentismo_porc === "" ? null : Number(formulario.presentismo_porc),
      usuario_accion: 1,
    };
    ...
    try {
      ...
      if (editandoId) {
        await alumnoComisionService.actualizar(editandoId, payload);
      } else {
        await alumnoComisionService.crear(payload);
      }
  ...
  async function eliminarRegistro(id) {
    const confirmar = confirm("Seguro que queres eliminar este alumno de la comision?");
    ...
    try {
      await alumnoComisionService.eliminar(id);
      await cargarDatos();
    } catch (err) {
      setError(err.message || "No se pudo eliminar el alumno de la comision");
    }
  }
  ...
  const registrosFiltrados = registros.filter((registro) => {
    ...
    const comision = mapas.comisiones[registro.comision_id_comision] || "";
    const condicion = mapas.condiciones[registro.condicion_academica_id] || "";

    return (
      legajo.toLowerCase().includes(textoBusqueda) ||
      comision.toLowerCase().includes(textoBusqueda) ||
      condicion.toLowerCase().includes(textoBusqueda)
    );
  });
        ...
        <section className="bg-white rounded-2xl shadow-md border border-slate-200 p-8">
            ...
            <div className="flex items-start gap-5">
              ...
              <div>
                <h1 className="text-4xl font-extrabold text-slate-800">
                  Alumnos por comision
                </h1>
                <p className="text-slate-500 mt-2">
                  Inscripciones, condicion academica, nota final y presentismo.
                </p>
              </div>
            </div>
          ...
          <div className="relative w-full md:w-96 mb-8">
            ...
            <input
              ...
              placeholder="Buscar por legajo, comision o condicion"
          ...
          <div className="lg:hidden space-y-4">
            {cargando ? (
              <EstadoVacio texto="Cargando alumnos por comision..." />
            ) : registrosFiltrados.length > 0 ? (
              registrosFiltrados.map((registro) => (
                <article
                  key={registro.id_historia_alumnos}
                ...
                >
                  <div className="mb-4">
                    ...
                    <h2 className="text-xl font-extrabold text-slate-800 mt-1">
                      {mapas.legajos[registro.legajo_id] || "-"}
                    </h2>
                  </div>

                  <div className="grid grid-cols-2 gap-3 text-sm">
                    <Dato label="Comision" value={mapas.comisiones[registro.comision_id_comision]} />
                    <Dato label="Condicion" value={mapas.condiciones[registro.condicion_academica_id]} />
                    <Dato label="Nota" value={formatearNumero(registro.nota_final)} />
                    <Dato label="Presentismo" value={formatearPorcentaje(registro.presentismo_porc)} />
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-2 mt-5 pt-4 border-t border-slate-200">
                    ...
                    <BotonAccion onClick={() => eliminarRegistro(registro.id_historia_alumnos)} tipo="eliminar" />
                  </div>
                </article>
              ))
            ) : (
              <EstadoVacio texto="No se encontraron alumnos por comision." />
            )}
          </div>
              ...
              <tbody>
                {cargando ? (
                  ...
                  registrosFiltrados.map((registro) => (
                    ...
                    >
                      <Td destacado>{mapas.legajos[registro.legajo_id] || "-"}</Td>
                      <Td>{mapas.comisiones[registro.comision_id_comision] || "-"}</Td>
                      <Td>{mapas.condiciones[registro.condicion_academica_id] || "-"}</Td>
                      <Td>{formatearNumero(registro.nota_final)}</Td>
                      <Td>{formatearPorcentaje(registro.presentismo_porc)}</Td>
                      <Td>
                  ...
                  <tr>
                    <td colSpan="6" className="text-center px-5 py-10 text-slate-500">
                      No se encontraron alumnos por comision.
                    </td>
                  </tr>
                )}
              </tbody>
            ...
            <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
              <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-2xl w-full max-w-4xl relative max-h-[92vh] overflow-y-auto">
                ...
                <h2 className="text-2xl font-extrabold text-slate-800 mb-5">
                  {editandoId ? "Editar alumno por comision" : "Nuevo alumno por comision"}
                </h2>
                ...
                <form onSubmit={guardarRegistro} className="space-y-5">
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <CampoSelect
                      ...
                      name="comision_id_comision"
                      value={formulario.comision_id_comision}
                      onChange={manejarCambio}
                    ...
                    <CampoSelect
                      ...
                      name="condicion_academica_id"
                      value={formulario.condicion_academica_id}
                      onChange={manejarCambio}
                      icon={<ClipboardCheck size={20} />}
                    >
                      <option value="">Seleccione</option>
                      {condicionesAcademicas.map((condicion) => (
                        <option
                          key={condicion.id_condicion_academica}
                          value={condicion.id_condicion_academica}
                        >
                          {condicion.condicion}
                        </option>
                      ))}
...
function validarPayload(payload) {
  ...
  if (!payload.comision_id_comision) {
    return "Debe seleccionar una comision asignatura";
  }

  if (!payload.condicion_academica_id) {
    return "Debe seleccionar una condicion academica";
  }

  if (payload.nota_final !== null && (payload.nota_final < 0 || payload.nota_final > 10)) {
    return "La nota final debe estar entre 0 y 10";
  }

  if (
    payload.presentismo_porc !== null &&
    (payload.presentismo_porc < 0 || payload.presentismo_porc > 100)
  ) {
    return "El presentismo debe estar entre 0 y 100";
  }
...
function obtenerMensajeError(err) {
  ...
  return err.message || "No se pudo guardar el alumno de comision";
}

export default AlumnosComision;
