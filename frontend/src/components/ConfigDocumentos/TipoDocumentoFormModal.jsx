...
export default function TipoDocumentoFormModal({
  mostrarModal,
  cerrarModal,
  modoEdicion,
  form,
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-2xl w-full max-w-lg relative animate-in fade-in zoom-in duration-200">
        ...
        <h2 className="text-2xl font-extrabold text-slate-800 mb-5">{modoEdicion ? "Editar Tipo de Documento" : "Nuevo Tipo de Documento"}</h2>
        
        <form onSubmit={guardar} className="space-y-4">
          <div>
            <label className="block text-sm font-bold text-slate-700 mb-2">Descripción Oficial</label>
            <input type="text" name="descripcion" value={form.descripcion} onChange={manejarCambio} placeholder="Ej: DNI, Pasaporte, Cédula" className={`w-full h-12 border ${errors.descripcion ? "border-red-500" : "border-slate-300"} rounded-xl px-4 text-slate-700 focus:outline-none focus:ring-2 focus:ring-red-500`} />
